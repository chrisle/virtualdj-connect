...
describe('VirtualDjM3uParser', () => {
  ...
  describe('constructor', () => {
    ...
    it('uses custom basePath when provided', () => {
      const parser = new VirtualDjM3uParser({ basePath: '/custom/path' });
...
`);
  ...
  describe('checkWriteHistory', () => {
    ...
    it('returns true when settings.xml does not exist', () => {
      const parser = new VirtualDjM3uParser({ basePath: '/test/path' });
      mockExistsSync.mockReturnValue(false);
    ...
    it('returns true when writeHistory is yes', () => {
      const parser = new VirtualDjM3uParser({ basePath: '/test/path' });
      mockExistsSync.mockReturnValue(true);
      mockReadFileSync.mockReturnValue('<writeHistory>yes</writeHistory>');
      expect(parser.checkWriteHistory()).toBe(true);
    });
    ...
    it('returns false when writeHistory is no', () => {
      const parser = new VirtualDjM3uParser({ basePath: '/test/path' });
      mockExistsSync.mockReturnValue(true);
      mockReadFileSync.mockReturnValue('<writeHistory>no</writeHistory>');
      expect(parser.checkWriteHistory()).toBe(false);
    });

    it('returns true when writeHistory tag is missing', () => {
      const parser = new VirtualDjM3uParser({ basePath: '/test/path' });
      mockExistsSync.mockReturnValue(true);
      mockReadFileSync.mockReturnValue('<otherSetting>value</otherSetting>');
      expect(parser.checkWriteHistory()).toBe(true);
    });
  });
});
